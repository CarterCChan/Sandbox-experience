...
function set_vertex_attrib_to_buffer(
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
        throw new Error('Shader program is not linked correctly.');
    }

    // Get the attribute location
    let attr_loc = gl.getAttribLocation(program, attrib_name);
    if (attr_loc == -1) {
        throw new Error('Attribute "' + attrib_name + '" not found in the shader program.');
    }

    // Ensure the buffer is valid
    if (!buffer) {
        throw new Error('Buffer is invalid or not created correctly.');
    }
    ...
    // Specify the data format for the attribute
